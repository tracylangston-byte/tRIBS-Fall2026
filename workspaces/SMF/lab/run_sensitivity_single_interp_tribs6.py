...
calibration_work/current_run_config.json, run from the lab/ directory.
...
def run_and_score():
    ...
    # ------------------------------------------------------------------
    script_dir = Path.cwd()
    project_root = script_dir.parent  # always run one level below project_root
    ...
    if synth_file is not None:
        ...
        # Two .qout formats can show up here: old tRIBS (pre-6.0.0) writes
        # tab/whitespace-delimited data under a header whose OWN labels
        # contain commas (e.g. "1-Time,hr"), while tRIBS 6.0.0 writes a
        # clean comma-delimited file with a real header ("Time_hr,Qstrm_m3_s,
        # Hlev_m") -- confirmed against both an old Series 100 truth file and
        # a new-build output, 2026-09-24. The header can't be used to tell
        # them apart (both contain commas), so sniff the first DATA line
        # instead: old-format data rows have zero commas, new-format rows
        # have exactly two.
        with open(synth_file) as _f:
            _f.readline()               # header
            _first_data_line = _f.readline()

        if ',' in _first_data_line:
            # New format (tRIBS 6.0.0): comma-delimited, real header row.
            # header=0 discards the file's own header regardless of its
            # exact column-name spelling (e.g. Qstrm_m3_s vs Qstrm_m3s);
            # names= then applies what the rest of this script expects.
            obs_raw = pd.read_csv(
                synth_file, sep=',', header=0,
                names=['Time_hr', 'Qstrm_m3s', 'Hlev_m']
            )
        else:
            # Old format (pre-6.0.0): whitespace/tab-delimited, no usable
            # header (its labels contain commas but aren't real delimiters).
            obs_raw = pd.read_csv(
        ...
        obs_filepath = '../init_data/met/SMF_Observations_1993-2025.xlsx'
