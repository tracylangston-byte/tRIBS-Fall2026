...
Usage (run from the lab/ directory, one level below project_root):
...
def build_input_file(param_name, value, overrides=None, tag="", gauge_sdf=None):
    ...
    gauge_sdf : str, optional
        Override path (relative to the lab/ directory) to the rain
        ...
        instead of the baseline ../init_data/met/Master_Precip.sdf.
    ...
    script_dir = Path.cwd()
    project_root = script_dir.parent  # always run one level below project_root (see docstring)
    ...
    # Built from the classified soil-ID raster rather than a pre-existing
    # soils.sdt template -- every field gets overwritten by SOIL_PARAM_LOOKUP
    # below regardless, so this only needs to supply the correct class IDs,
    # which it derives directly from soil_ras itself.
    ...
    # textures=False: no per-class Texture labels were supplied to
    # create_soil_table_from_map(), so True here would KeyError on
    # the reference-only textures sidecar (not read by tRIBS anyway).
    ...
    met.gaugestations['value']    = gauge_sdf if gauge_sdf else "../init_data/met/Master_Precip.sdf"

    # Model class
    model = Model(met=met, land=land, soil=soil, mesh=None, meta=proj.meta)

    # Mesh: pre-generated
    # NOTE: inputtime removed -- no longer exists as a Model attribute in
    # pytRIBS 1.0.0 (confirmed empirically, not just absent from grep); the
    # mesh-input keyword group in this version only has optmeshinput and
    # inputdatafile (+ pointfilename, unused here). Flag for Josh if the
    # ported mesh read behaves unexpectedly.
    ...
    # NOTE: outhydrofilename removed -- no longer exists as a Model attribute
    # in pytRIBS 1.0.0 (confirmed empirically). outfilename + outletnodelist
    # (set below) are very likely what now determines the .qout name per the
    # tRIBS docs' description of outlet-file naming, but this is inferred,
    # not run-confirmed -- worth a specific check once the smoke test runs.
    ...
    # Solar position (CENTROIDLAT/CENTROIDLONG/UTCOFFSET) -- new requirement in
    # tRIBS 6.0.0 / pytRIBS 1.0.0. Derived from the actual watershed boundary,
    # dissolved from the mesh's Voronoi cell polygons (confirmed against SMF_voi.shp:
    # pure Polygon geometry, 3398 features -- one per mesh node -- vs. SMF_stream.shp,
    # which is the LineString channel network, not a boundary). Centroid verified
    # 2026-09-24 at 33.327N, -112.114W -- South Mountain Park, Phoenix, AZ, as expected.
    voi = gpd.read_file('../init_data/shapefiles/SMF_voi.shp')
    try:
        watershed_boundary = voi.union_all()       # geopandas >=0.14
    except AttributeError:
        watershed_boundary = voi.unary_union       # older geopandas
    ...
    current_run_config = {
        ...
        "event_end":                 EVENT_END,
        "gauge_sdf":                 gauge_sdf if gauge_sdf else "../init_data/met/Master_Precip.sdf",
        "Ks_mult":                   Ks_mult,
        "f_RS_abs":                  f_RS_abs,
        "As_value":                  As_value,
        "Au_value":                  Au_value,
